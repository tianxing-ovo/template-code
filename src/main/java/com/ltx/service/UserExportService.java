...
public class UserExportService {
     ...
     */
    @SneakyThrows
    public <T> void export(HttpServletResponse response, List<T> list, ExportRequest exportRequest, Class<T> clazz) {
        List<String> fieldList = exportRequest.fieldList();
        // 设置响应内容类型和字符集
        response.setContentType(ExcelUtil.XLSX_CONTENT_TYPE + ";charset=UTF-8");
        String timeStamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        String fileName = URLEncoder.encode("用户数据_" + timeStamp + ".xlsx", StandardCharsets.UTF_8);
        response.setHeader(HttpHeaders.CONTENT_DISPOSITION, "attachment;filename=" + fileName);
        EasyExcel.write(response.getOutputStream(), clazz).excelType(ExcelTypeEnum.XLSX).registerWriteHandler(new CustomCellStyleStrategy()).registerWriteHandler(new CustomCellWriteHandler()).includeColumnFieldNames(fieldList).orderByIncludeColumn(true).sheet().doWrite(list);
    }
     ...
     * @param exportRequest 导出请求参数
     */
    private <T> void exportToLocal(List<T> list, String fileKey, ExportRequest exportRequest) {
        List<String> fieldList = exportRequest.fieldList();
        Path targetPath = fileStorageProperties.getStoragePath().resolve(fileKey).normalize();
        FileUtil.mkdir(targetPath.getParent());
        EasyExcel.write(targetPath.toFile(), User.class).excelType(ExcelTypeEnum.XLSX).registerWriteHandler(new CustomCellStyleStrategy()).registerWriteHandler(new CustomCellWriteHandler()).includeColumnFieldNames(fieldList).orderByIncludeColumn(true).sheet().doWrite(list);
    }
     ...
     * @return 导出任务实体
     */
    public ExportTask createPendingTask(Integer userId) {
        // 生成按日期划分的导出子目录
        String dateSubDir = "export/" + LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));
        // 组装最终相对存储标识
        String fileKey = StrUtil.format("{}/user-{}.xlsx", dateSubDir, IdUtil.fastSimpleUUID());
        // 创建导出任务实体
        ExportTask exportTask = new ExportTask();
        exportTask.setUserId(userId);
        exportTask.setFileKey(fileKey);
     ...
     * @param exportRequest 导出请求参数
     * @param taskId        任务ID
     * @param fileKey       文件相对路径标识
     */
    @Async
    public void asyncExport(ExportRequest exportRequest, Long taskId, String fileKey) {
        // 更新任务状态为⌈导出中⌋
        ...
        try {
            ...
            // 导出到本地
            exportToLocal(userList, fileKey, exportRequest);
            // 获取文件大小和记录数
            File file = fileStorageProperties.getStoragePath().resolve(fileKey).toFile();
